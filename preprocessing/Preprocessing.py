...
class Preprocessing():
    ...
    def __init__(self, pp_config, dict_df_posts, log) -> None:
    ...
    def preprocessing(self) -> None:
        ...
        # for each dataframe of posts, create a process for preprocessing
        # each dataframe may be preprocessed with n cores
        processes = [Process(target=self.preprocessing_manager, args=(key, df, n_cores, queue_preprocessing)) for key, df in self._dict_df_posts.items()]

        # start the processes
        for p in processes:
            p.start()

        # update the original dictionary of posts
        for _ in processes:
            ...
            self._dict_df_posts[k] = df_processed
